...
import cardmoto from '../../img/card-moto.png';
...
import {
    ...
    apiGetModelsByBrandNameYearMoto,
    apiGetYearsByCodia,
    apiLeadsCompleteFields
} from '../../services/main';
...
const MotoPolicy = (props) => {
    ...
    useEffect(() => {
        ...
        amplitudeLogEvent("Moto/Policy");

        if (props.match.params.leadID && props.match.params.vehicleID) {
    ...
    useEffect(() => {
        ...
        if (step === 2) {
            ...
            apiGetModelsByBrandNameYearMoto({ brand: make || data.state.brand })
                .then(res => {
        ...
        if (step === 3) {
            ...
            if (data.state && data.state.brand && data.state.version) {
                ...
                apiGetModelsByBrandNameYearMoto({ brand: make || data.state.brand })
                    .then(res => {
                        ...
                        apiGetYearsByCodia({ codia: res.filter(i => removeSeveralSpace(i.name) === removeSeveralSpace(version || data.state.version))[0].id, type: "m" })
                            .then(res => {
                ...
                apiGetYearsByCodia({ codia: versionlist.filter(i => removeSeveralSpace(i.name) === removeSeveralSpace(version))[0].id, type: "m" })
                    .then(res => {
                        console.log("years-----", res);
    ...
    useEffect(() => {
        if (step === 1) {
            if (!loadingBrand) {
                ...
                if (props.match.params.leadID && props.match.params.vehicleID && ((!loadingVehicle && !loadingLead) || (!loadingCodia && !loadingLead)) && !loadingBrand) {
                    setLoading(false);
                } else {
                        ...
                        <div className="w-full pl-4 sm:pl-8 pt-15">
                            <div className="flex items-center">
                                <div className="flex items-center cursor-pointer" onClick={() => setStep(step - 1 === 0 ? 1 : step - 1)}>
                                    ...
                                    <p className="pl-2 text-purple font-OpenSansSemiBold text-lg">Paso {step} de 4</p>

                                </div>
                                {((step === 1 && !!make) || (step === 2 && !!year) || (step === 3 && !!version)) &&
                                    ...
                                    <>
                                        ...
                                        <div className="pr-4 sm:pr-16 lg:pr-32 1xl:pr-64">
                                                ...
                                                <div className="w-full mt-2 py-2 pl-4 pr-3 bg-white rounded-xl">
                                                    <div className="w-full model-select bg-white pb-4" style={{ maxHeight: "456px" }}>
                                                        ...
                                                        {versionlist.filter(i => i.name.toLocaleLowerCase().includes(versionfilter.toLocaleLowerCase())).map(item => (
                                                            <ModelItemMoto key={item.id} name={item.name} item={item.name} group={version} onClick={() => { setVersion(item.name); setTimeout(function () { setStep(3) }, 500); }} />
                                                        ))}
                                            ...
                                            <div className="pt-4 flex items-center float-right cursor-pointer" onClick={() => setModalIsOpen(true)}>
                                                <p className="font-OpenSansSemiBold text-base sm:text-xl text-purple">¿Cuál es la versión de mi moto?</p>
                                                <img src={right1} alt="" />
                                            </div>
                                            ...
                                            >
                                                <div className="w-full relative rounded-2xl px-4 sm:px-8">
                                                    ...
                                                    <div className="px-0 md:px-24 pt-10 pb-6 sm:pb-10">
                                                        ...
                                                        <img src={cardmoto} alt="card-moto" className="w-full" />
                                                    </div>
                                                </div>
                                            </Modal>
                                        </div>
